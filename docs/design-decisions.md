...
**Choice** — Full calendar-anchoring for onboarding, an overall whole-period budget that sits alongside (not instead of) per-category ones, and a cadence switch that takes effect immediately by truncating the current period rather than waiting for a future boundary.
...
*Why a switch is instant, reversing DEC-008's own earlier rejection of that.* The first version of this decision made a switch wait for the next real Monday or 1st — sometimes close to three weeks away, for a switch to monthly initiated mid-month. That felt broken rather than careful, and was corrected before it ever ran on a device: an instant switch is what was actually wanted, DEC-008's "partial periods never exist" notwithstanding. What made the wait feel necessary in the first place — a switch that silently mangles the period the user is in — is solved by the confirmation screen itself, not by delay: the screen shows exactly what will change, including a fresh budget for the new period, before anything is written. Truncating a period the user can see being truncated, with the new numbers on the same screen, is not the invisible surprise DEC-008 was written to prevent.

*Why the current period is truncated, not extended into a bridge.* The switch takes effect on the day the user confirms it: today belongs to the new cadence, and the old period ends yesterday. "Monday to Sunday" is therefore a promise about a period's *shape* once it starts, not a guarantee that every switch itself lands on a Monday — switching to weekly on a Wednesday produces a Wednesday-to-Tuesday week from then on, not a retroactive jump back to Monday. The alternative — snapping back to the current calendar week or month — was rejected because it would reclassify days, and the spending already logged against them, from the old period into the new one after the fact. Nothing already logged should ever change which period it belongs to as a side effect of a decision made later.

*The one edge case worth naming.* If the current period itself already started today — a second switch on the same day, or the very first period of a fresh install — there is no "yesterday" inside it to truncate to, and writing one would violate the schema's own `ends_on >= starts_on` check. That row is retired (invariant 3's tombstone) rather than truncated. Nothing is lost: spending already logged today is dated, not linked to a period id, so it is picked up automatically once the new period exists to claim that date — the same reattribution-by-date-range that makes the ordinary truncation case safe in the first place.

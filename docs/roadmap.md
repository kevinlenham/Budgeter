...
**Stack** — Expo + TypeScript, raw SQLite via `expo-sqlite`, EAS Build for cloud compilation, TestFlight to device. See DEC-032 for why this supersedes DEC-001, and what it cost.
...
**Goal** An Expo app running on your actual iPhone, with a test suite that runs.

- `create-expo-app` with the TypeScript template, strict mode on
- `expo-sqlite`, `expo-file-system`, `expo-local-authentication` installed
- **A dev client build, not Expo Go.** Expo Go cannot load the native modules this project will need. Building the dev client once now means every later native addition is an incremental rebuild rather than a migration.
- EAS configured; first cloud build installed on your phone
- Vitest (or Jest) + `fast-check` for property tests, ESLint, Prettier
- GitHub Actions running typecheck, lint and tests on push — plain Ubuntu runners, no macOS needed
- Apple Developer Program enrolment started (USD $99/yr, non-optional per DEC-032 — it takes a day or two to process, so start it now rather than at Sprint 3)

**Done when** a trivial failing test fails in CI, and the dev client app opens on your phone and hot-reloads a text change from your Windows machine.
...
- `Money`: a branded TypeScript type over integer minor units + currency code. No arithmetic operators — explicit `add`/`subtract`/`allocate` functions, currency mismatch throws (invariant 1)
- Property tests with `fast-check`: associativity, no precision loss, currency mismatch rejected, `allocate` sums exactly to the input with no lost cents
...
- CHECK constraints: `status IN ('draft','confirmed')`, transfer rows have NULL `category_id`, split parents have NULL `category_id`
...
- The **`spending` view** (DEC-010) and the **`postings` view** (DEC-028)
...
**Watch for**
- `PRAGMA foreign_keys = ON` is off by default in SQLite and must be set per connection. Easy to forget, silently disables half the schema.
- Use the `uuid` package's v7, not v4 — DEC-006 wants the timestamp ordering for index locality. Test that property.
- Tests should run against in-memory SQLite in Node, not on device. That keeps this entire sprint on the fast loop.

---

## Sprint 2 — Periods and limits

**Estimate** 1–2 sessions. Pure logic, no UI, and it all runs in Node.
...
**Watch for** JavaScript `Date` is a genuine hazard for exactly this work — it is a UTC instant wearing a local-time costume, which is the precise confusion DEC-009 exists to prevent. Use a date-only library (`@internationalized/date` or `temporal-polyfill`) for boundaries, and never let a `Date` object near `booked_on`.
...
- Onboarding: accounts, next payday (DEC-007), cadence, starter categories
- Add/edit transaction form
- Ledger list grouped by day, reading the `spending` view, built on `FlashList`
- Budget screen: per-category "$340 of $500", days remaining, safe-to-spend
- **Resolve the reactivity open item.** GRDB's `ValueObservation` has no direct equivalent (DEC-032). Simplest defensible answer: TanStack Query over the SQLite reads, with writes invalidating the affected keys. Decide it here, against a real screen, and write it down.
...
**Note** DEC-001's warning about `List` degrading on complex rows is struck by DEC-032; `FlashList` handles this shape well. Still do not pre-optimise.

---

## Sprint 4 — Categories, merchant memory, and export

**Estimate** 2 sessions

**Goal** Categorisation stops being tedious, and the data becomes genuinely durable.
...
- **JSON/CSV export** via `expo-file-system` + `expo-sharing`. DEC-002 chose durability over sync and explicitly notes that durability *requires the export feature to actually be built, not assumed*. Until this ships, the no-sync decision is unbacked and you have one copy of the data and a hope.
- Round-trip test: export, then re-import your own export

**Done when** exporting and re-importing produces an identical dataset with zero duplicate rows — which also exercises the Sprint 1 funnel.
...
- File pick via `expo-document-picker`
...
**Estimate** 4–5 sessions. **Highest risk in the project, and DEC-032 raised it.**

This is the one sprint where the Windows constraint genuinely hurts: the App Intent is Swift, wrapped as an Expo config plugin around a native target, and neither of us can compile it locally. Every iteration is a cloud build. Budget accordingly and expect the first working capture to take longer than feels reasonable.

- Expo config plugin adding an App Intents extension target
...
**Estimate** 3–4 sessions. Two items here got harder under DEC-032.

- **Privacy overlay** on the `AppState` `inactive` transition (DEC-025). This is an open item, not a known quantity — verify on a physical device that it renders before iOS takes the app-switcher screenshot. If it cannot be made reliable in JS, it becomes a small native module, and it is worth that cost: it is the leak most apps get wrong.
- Face ID after N seconds backgrounded via `expo-local-authentication`, configurable, default 1 minute
...
- **Widgets** with Lock Screen amounts hidden by default (DEC-025). WidgetKit is Swift-only, so this is a second native extension target, cloud-built. Consider deferring past v1 if Sprint 7 has already exhausted your patience for blind Swift.
...
**Sprints 0–5 are v1.** That is already a good budgeting app: manual entry, budgets that respect real pay cycles, transfers, splits, categorisation memory, and exportable data. All of it is TypeScript on the fast loop — no blind Swift anywhere.
...
Write the TypeScript, the SQL migrations, the views and triggers, and the tests. Draft each sprint on its own branch. Explain the reasoning wherever a decision wasn't already settled in the design doc — and flag it explicitly when implementation reveals a recorded decision was wrong, so it goes back into `design-decisions.md` as a new DEC rather than getting quietly worked around. DEC-032 is the template for that.

### What only you can do

- **Run the app.** I can write the code and reason about it, but I can't see your phone. Screenshots are worth a lot.
- **Apple Developer Program, EAS account, TestFlight** — anything behind a login or a payment
...
3. You run `npm test` and paste failures verbatim — full output, not summarised. TypeScript errors in particular carry information that gets lost in paraphrase.
...
- Keep the fast loop fast. Anything testable in Node should be tested in Node, not on the device. Sprints 1, 2 and 8 should never need a build.
